...
public class DatabaseInitializerConfig {
    ...
    public CommandLineRunner initDatabase(
        ...
        return args -> {
            ...
            System.out.println(">> Cargando permisos y roles...");

            // ===== PERMISOS =====

            PermitEntity verOfertas = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.VER_OFERTAS)
                            .build());

            PermitEntity crearOferta = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.CREAR_OFERTA)
                            .build());

            PermitEntity editarOferta = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.EDITAR_OFERTA)
                            .build());

            PermitEntity eliminarOferta = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.ELIMINAR_OFERTA)
                            .build());

            PermitEntity postularseOferta = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.POSTULARSE_OFERTA)
                            .build());

            PermitEntity verPostulaciones = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.VER_POSTULACIONES)
                            .build());

            PermitEntity gestionarPostulaciones = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.GESTIONAR_POSTULACIONES)
                            .build());

            PermitEntity enviarMensaje = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.ENVIAR_MENSAJE)
                            .build());

            PermitEntity leerMensajes = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.LEER_MENSAJES)
                            .build());

            PermitEntity administrarSistema = permitRepository.save(
                    PermitEntity.builder()
                            .permit(Permits.ADMINISTRAR_SISTEMA)
                            .build());
